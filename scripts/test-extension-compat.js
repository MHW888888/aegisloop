...
assert.match(content, /separate AegisLoop local bridge route/, 'contract must distinguish AegisLoop from built-in Codex');
assert.match(content, /Do not start or invoke ChatGPT's built-in Codex/, 'contract must prevent accidental built-in Codex dispatch');
assert.match(content, /Execution route/, 'panel must show which execution route is active');
assert.match(content, /AegisLoop local bridge/, 'panel must identify the local bridge route');
