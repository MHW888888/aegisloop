...
Use this page when recruiting new testers and contributors instead of assigning work to existing contributors.
...
Avoid asking new contributors to understand every bridge, turn token, capsule, ACK/NACK, and Codex detail before they can help.
...
Goal: changing GPT-5.5 / 5.4 / 5.3 / o3 should keep the same ChatGPT conversation -> same local Codex route.
...
AegisLoop 是一个本地优先的 ChatGPT -> Codex 自动化桥：ChatGPT 负责规划，本地 Codex 负责执行，中间有显式授权、运行隔离、ACK/NACK 和审计日志。
